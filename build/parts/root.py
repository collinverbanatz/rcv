"""
“This setup creates the full rig hierarchy (MODEL/GEO, RIG/CTRL, and SKEL).
It builds the root controls, creates the root joint under the SKEL group, and prepares the character so the mesh can be skinned and bound later.”
"""
...
import importlib
importlib.reload(nmModule)


class Root(nmModule.RigModule):
    def __init__(self,
                 side=None,
                 part=None,
                 guide_list=None,
                 ctrl_scale=None,
                 global_shape='gear_2D',
                 root_shape='circle',
                 model_path=None,
                 guide_path=None):

        # ✅ MUST exist before RigModule.__init__()
        if guide_list:
    ...
    def control_rig(self):
        ...
        self.global_ctrl = nmCtrl.Control(parent=self.control_grp,
        ...
        self.root_01 = nmCtrl.Control(parent=self.global_ctrl.ctrl,
        ...
        self.root_02 = nmCtrl.Control(parent=self.root_01.ctrl,
                                      shape=self.root_shape,
                                      prefix=self.side,
                                      suffix='CTRL',
                                      name='root_02',
    ...
    def skeleton(self):
        self.bind_joints = []
        root_chain = nmChain.Chain(transform_list=[self.root_jnt],
                                   prefix=self.side,
                                   suffix='JNT',
                                   name=self.part)
