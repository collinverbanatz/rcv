...
import nmrig.libs.transform as nmXform
import nmrig.libs.maths as nmMath
# import nmrig.libs.maths as nmMath
import importlib

importlib.reload(nmCtrl)
importlib.reload(nmCommon)
importlib.reload(nmAttr)
importlib.reload(nmXform)
# importlib.reload(nmMath)
...
class Chain:
    ...
    def create_from_curve(self, joint_num=5, curve=None, aim_vector=(0, 1, 0),
        ...
        for i in range(joint_num):
            ...
            pos = nmXform.find_position_on_curve(curve, i * inc)
    ...
    def create_from_transforms(self,
                               ...
                               static=False,
                               pad='auto'):
        pose_dict = nmXform.read_pose(self.transform_list)
        if pad == 'auto':
        ...
        for i, pose in enumerate(pose_dict):
            ...
            nmXform.set_pose(jnt, pose_dict[pose])
            self.joints.append(jnt)
    ...
    def create_blend_chain(self,
        ...
        self.switch = nmAttr.Attribute(node=switch_node, type='double', min=0,
    ...
    def bend_chain(self, bone, ctrl_scale, spans=16, mirror=False, global_scale=None):
        ...
        pos_list.append(e)
        b_crv = cmds.curve(
            point=pos_list, degree=3, bezier=True, knot=[0, 0, 0, 1, 1, 1, 2, 2, 2]
        )
        ...
        for i, jnt in enumerate(seg_jnt_list):
            ...
            pci = cmds.createNode("pointOnCurveInfo", name=jnt.replace("JNT", "PCI"))
            cmds.connectAttr(j_crv + "Shape.worldSpace[0]", pci + ".inputCurve")
        ...
        for i, jnt in enumerate(seg_jnt_list):
            if jnt != seg_jnt_list[-1]:
                ...
                next_loc = end_jnt
            dist = cmds.createNode("distanceBetween", name=jnt.replace("JNT", "DST"))
            mdn = cmds.createNode("multiplyDivide", name=jnt.replace("JNT", "MDN"))
            cmds.setAttr(mdn + ".operation", 2)
            cmds.connectAttr(loc_list[i] + ".worldMatrix[0]", dist + ".inMatrix1")
            cmds.connectAttr(next_loc + ".worldMatrix[0]", dist + ".inMatrix2")
            cmds.connectAttr(dist + ".distance", mdn + ".input1X")
            d = cmds.getAttr(dist + ".distance")

            if global_scale:
                mdl = cmds.createNode("multDL", name=jnt.replace("JNT", "MDL"))
            ...
            cmds.connectAttr(mdn + ".outputX", jnt + ".scaleY")

        # organize
        rig_grp = cmds.group(
            b_crv, j_crv, loc_list, name=bone.replace("JNT", "bendy_rig_GRP")
        )
        ...
        for attr in ["translate", "rotate", "scale"]:
            cmds.connectAttr(dcm + ".output" + attr.capitalize(), ctrl_grp + "." + attr)
        ...
        attr_util.lock_and_hide(
            node=mid_ctrl.ctrl, translate=False, rotate=False, scale="XZ"
        )
        ...
        cmds.xform(s_tan.control_dict["rig_groups"][0], worldSpace=True, pivots=s)
        cmds.xform(e_tan.control_dict["rig_groups"][0], worldSpace=True, pivots=e)

        # deform the curves
        cmds.wire(
            j_crv,
            wire=b_crv,
            dropoffDistance=[0, 5000],
            crossingEffect=0,
            localInfluence=0,
            name=j_crv + "_wire",
        )
        ...
        cmds.cluster(
            b_crv + ".cv[0]",
            bindState=True,
            weightedNode=(bone, bone),
            name=b_crv + "_start_CLS",
        )
        # start tangent
        cmds.cluster(
            b_crv + ".cv[1]",
        ...
        sb_cls = cmds.cluster(b_crv + "BaseWire.cv[1]", name=b_crv + "_base_start_CLS")[
            1
        ]
        eb_cls = cmds.cluster(b_crv + "BaseWire.cv[5]", name=b_crv + "_base_end_CLS")[1]
    ...
    def get_chain_lengths(self):
        ...
        for i in range(len(self.joints) - 1):
            ...
            bone_len = nmMath.distance_between(point_a=jnt_a, point_b=jnt_b)
